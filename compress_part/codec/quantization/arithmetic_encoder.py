...
def encode_and_decode(data, num_bins, file_name_prefix, bin_dir):
    ...
    end_encoding_time = time.time()  # 结束计时编码过程
    encoding_time = end_encoding_time - start_encoding_time  # 计算编码时间

    start_decoding_time = time.time()  # 开始计时解码过程
    # 步骤 7: 解码
    ...
    end_decoding_time = time.time()  # 结束计时解码过程
    decoding_time = end_decoding_time - start_decoding_time  # 计算解码

    return np.array(decoded_data), encoding_time, decoding_time
...
def main():
    ...
    for name, shape in names_shapes.items():
        ...
        decoded_data, encoding_time, decoding_time = encode_and_decode(data.flatten(), num_bins, name, bin_dir)
    ...
    print(f"总编码时间: {total_encoding_time:.6f}秒")
    print(f"总解码时间: {total_decoding_time:.6f}秒")
    # 保存为 npz
    arrays_to_save = {}
    for name, shape in names_shapes.items():
        decode_txt_path = os.path.join(txt_dir, f'decode/{name}.txt')
