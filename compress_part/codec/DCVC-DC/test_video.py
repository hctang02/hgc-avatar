...
def run_test(p_frame_net, i_frame_net, args):
    ...
    overall_i_encoding_time = 0
    overall_i_decoding_time = 0
    with torch.no_grad():
        for frame_idx in range(frame_num):
            ...
            if frame_idx % gop_size == 0:
                ...
                print(f"I-frame encoding time: {result['encoding_time']:.3f} seconds, I-frame decoding time: {result['decoding_time']:.3f} seconds")
                overall_i_encoding_time += result['encoding_time']
                overall_i_decoding_time += result['decoding_time']
                dpb = {
    ...
    print(f"Overall I-frame encoding time: {overall_i_encoding_time:.3f} seconds, Overall I-frame decoding time: {overall_i_decoding_time:.3f} seconds")
    if args['dist_in_yuv420']:
...
'''
python test_video.py --i_frame_model_path ./checkpoints/cvpr2023_image_psnr.pth.tar --p_frame_model_path ./checkpoints/cvpr2023_video_psnr.pth.tar --rate_num 2 --test_config ./dataset_config_rgb.json --yuv420 0 --cuda 1 --worker 1 --write_stream 1  --stream_path /mnt/ssd2tB/haocheng/AnimatableGaussians/compress_part/data/test/output-actor01-test/pose_map/bin --output_path output.json --force_intra_period 32 --force_frame_num 500 --save_decoded_frame 1 --decoded_frame_path /mnt/ssd2tB/haocheng/AnimatableGaussians/compress_part/data/test/output-actor01-test/pose_map/decode_frame --root_path /mnt/ssd2tB/haocheng/AnimatableGaussians/compress_part/data/test/output-actor01-test/
'''