...
class AvatarNet(nn.Module):
    ...
    def render(self, items, bg_color = (0., 0., 0.), use_pca = False, use_vae = False):
        ...
        # folder_output = "output-lbn2-train"
        folder_output = "output-test--nouse"
        
        # 获取当前时间戳
        # # 假设 pose_map 是一个 shape 为 (3, 512, 512) 的 tensor
        # pose_map_img = pose_map.permute(1, 2, 0)  # 转换维度到 (512, 512, 3)

        # # 归一化并转换数据类型为 uint8
        # pose_map_img = np.clip((pose_map_img.cpu().detach().numpy() * 255), 0, 255).astype(np.uint8)

        # plan 1 : 保存为.npy 格式
        # # 直接转换为 numpy 数组
        # pose_map_npy = pose_map.cpu().detach().numpy()
        # os.makedirs(f"{folder_output}/pose_map_npy", exist_ok=True)
        # npy_path = f"{folder_output}/pose_map_npy/pose_map_{items['item_idx']}.npy"
        # np.save(npy_path, pose_map_npy)
        # # plan 2 : 保存为img 形式
        # os.makedirs(f"{folder_output}/pose_map_img_new", exist_ok=True)
        # save_pose_map1 = pose_map.clone()
        # save_pose_map1.clip_(0., 1.)
        ...
        # import pandas as pd
        # pose_map_npy = pose_map.cpu().detach().numpy()
        # os.makedirs(f"{folder_output}/pose_map_csv", exist_ok=True)
        # df = pd.DataFrame(pose_map_npy.reshape(-1, pose_map_npy.shape[-1]))  # 拉平张量
        # df.to_csv(f"{folder_output}/pose_map_csv/pose_map_{items['item_idx']}.csv", index=False)
        save_pose_map1 = pose_map.clone().clip(0., 1.)  # 确保在 [0,1] 范围
        save_pose_map1 = (save_pose_map1 * 255).to(torch.uint8)  # 转为 uint8
        save_pose_map1 = save_pose_map1.permute(1, 2, 0).cpu().numpy()  # (H, W, C)
        file_path = f"{folder_output}/pose_map_img_new"
        if os.path.exists(file_path):
            print(f"文件保存成功：{file_path}")
        else:
            print(f"文件保存失败！请检查路径和权限：{file_path}")
        Image.fromarray(save_pose_map1).save(f"/mnt/ssd2tB/haocheng/AnimatableGaussians/output-test--nouse/pose_map_img_new/pose_map_{items['item_idx']}.png")
        # 读取图片posemap的代码部分
        # pose_map_img = cv.imread(f"pose_map_aftercompress/actor04/pose_map_{items['item_idx']}.jpg", cv.IMREAD_COLOR)
        # pose_map_img = cv.imread(f"/mnt/ssd2tB/haocheng/AnimatableGaussians/output-actor04-test/pose_map_img/pose_map_{items['item_idx']}.jpg", cv.IMREAD_COLOR)
        # pose_map_img = cv.imread(f"{folder_output}/pose_map_img_new/pose_map_{items['item_idx']}.png", cv.IMREAD_UNCHANGED)
        # pose_map_img = cv.cvtColor(pose_map_img, cv.COLOR_BGR2RGB)  # **转换回 RGB**
        # pose_map_tensor = torch.tensor(pose_map_img, dtype=torch.float32) / 255.0  # 归一化
        # pose_map_tensor = pose_map_tensor.permute(2, 0, 1)  # (H, W, C) -> (C, H, W)

        # # pose_map_img = cv.imread(f"{folder_output}/pose_map_img_new/pose_map_{items['item_idx']}.jpg", cv.IMREAD_COLOR)
        # # pose_map_tensor = torch.tensor(pose_map_img, dtype=torch.float32) / 255.0
        # # pose_map_tensor = pose_map_tensor.permute(2, 0, 1)
        # pose_map_tensor = pose_map_tensor.to(pose_map.device)
        # pose_map_tensor = pose_map_tensor.clamp(0., 1.)
        # 使用 PIL 加载
        loaded_img = Image.open(f"{folder_output}/pose_map_img_new/pose_map_{items['item_idx']}.png")
        pose_map_img = np.array(loaded_img)  # 直接得到 RGB numpy array

        # 转换回 tensor
        pose_map_tensor = torch.tensor(pose_map_img, dtype=torch.float32) / 255.0
        pose_map_tensor = pose_map_tensor.permute(2, 0, 1).to(pose_map.device)



        # # 加载保存的 numpy 数据
        # loaded_image_np = np.load(npy_path)
        # # 如果需要转回 tensor 格式，并保持原设备
        # pose_map_tensor = torch.tensor(loaded_image_np)
        # pose_map_tensor = pose_map_tensor.to(pose_map.device)
        # os.remove(npy_path)

        # def compress_image(image, folder_output, timestamp, quality=75):
        ...
        #     # Save the encoded compressed image
        #     output_dir = os.path.join(folder_output, 'pose_map_encode')
        #     os.makedirs(output_dir, exist_ok=True)  # Ensure the directory exists
            
        #     # Define the file path for saving the compressed image
        #     output_file = os.path.join(output_dir, f'pose_map_{timestamp}.jpg')
        ...
        # save_pose_map.clip_(0., 1.)
        # save_pose_map = (save_pose_map * 255).to(torch.uint8)
        # save_pose_map = save_pose_map.permute(1, 2, 0)
        
        # # Compress and save image
        # compressed_img = compress_image(save_pose_map.cpu().numpy(), folder_output, timestamp, quality=75)
        # # print("compressed_img.shape is ", save_pose_map.shape)
        # # 将 tensor 转换为 NumPy 数组
        # # compressed_img = save_pose_map.cpu().numpy()
        # # compressed_img = compressed_img.numpy()
        # # 确保保存时是 8 位图像
        # compressed_img = cv.cvtColor(compressed_img, cv.COLOR_RGB2BGR)
        # os.makedirs(f"{folder_output}/pose_map_decode", exist_ok=True)
        # cv.imwrite(f"{folder_output}/pose_map_decode/pose_map_{timestamp}.jpg", compressed_img)
        
        # # 2.加载compressed image
        # pose_map_img = cv.imread(f"pose_map_aftercompress/actor04/pose_map_{items['item_idx']}.jpg", cv.IMREAD_COLOR)
        # pose_map_img = cv.imread(f"/mnt/ssd2tB/haocheng/AnimatableGaussians/output-actor04-test/pose_map_img/pose_map_{items['item_idx']}.jpg", cv.IMREAD_COLOR)
        # pose_map_img = cv.imread(f"{folder_output}/pose_map_img_new/pose_map_{items['item_idx']}.jpg", cv.IMREAD_COLOR)
        # pose_map_tensor = torch.tensor(pose_map_img, dtype=torch.float32) / 255.0
        # # 确保恢复为 (3, 512, 512)
        # if len(pose_map_tensor.shape) == 2:  # 如果是灰度图 (H, W)
        #     # 转换为 (1, H, W)，然后扩展为 3 通道
        #     pose_map_tensor = pose_map_tensor.unsqueeze(0)  # 转换为 (1, H, W)
        #     pose_map_tensor = pose_map_tensor.repeat(3, 1, 1)  # 恢复为 (3, H, W)
            
        #     # 调整大小为 (3, 512, 512)
        #     pose_map_tensor = torch.nn.functional.interpolate(pose_map_tensor.unsqueeze(0), size=(512, 512), mode='bilinear', align_corners=False).squeeze(0)
        # pose_map_tensor = pose_map_tensor.permute(2, 0, 1)
        # pose_map_tensor = pose_map_tensor.to(pose_map.device)


        # # 3.附加：把compressed image再保存一遍，检查问题
        # save_pose_map_tensor = pose_map_tensor
        # save_pose_map_tensor.clip_(0., 1.)
        # save_pose_map_tensor = (save_pose_map_tensor * 255).to(torch.uint8)
        # save_pose_map_tensor = save_pose_map_tensor.permute(1, 2, 0)
        
        # print("save_pose_map_tensor.shape is ", save_pose_map_tensor.shape)
        # save_pose_map_tensor_np = save_pose_map_tensor.cpu().numpy()  # 转换为 NumPy 数组
        # pose_map_tensor_save = cv.cvtColor(save_pose_map_tensor_np, cv.COLOR_RGB2BGR)

        # cv.imwrite(f"{folder_output} /pose_map_next/pose_map_{timestamp}.jpg", pose_map_tensor_save)
        # print("pose_map_tensor_save.shape is ", pose_map_tensor_save.shape)
