...
class LPIPS(nn.Module):
    ...
    def forward_face(self, in0, in1, in2, item_idx, retPerLayer=False, normalize=False):
        if normalize:  # 归一化到 [-1, +1]
            in0 = 2 * in0 - 1
            in1 = 2 * in1 - 1
        alpha = 0.2
        in0_input, in1_input = (self.scaling_layer(in0), self.scaling_layer(in1)) if self.version == '0.1' else (in0, in1)
        outs0, outs1 = self.net.forward(in0_input), self.net.forward(in1_input)
        feats0, feats1, diffs = {}, {}, {}
        step_ratio = min(1, item_idx / 800000)

        for kk in range(self.L):
            feats0[kk], feats1[kk] = lpips.normalize_tensor(outs0[kk]), lpips.normalize_tensor(outs1[kk])
            diffs[kk] = (feats0[kk] - feats1[kk]) ** 2
            # print("diffs[kk].shape is", diffs[kk].shape)
            # print("in2.shape is", in2.shape)
            in2 = in2.float()
            if in2.shape[2:] != diffs[kk].shape[2:]:
                in2 = F.interpolate(in2, size=diffs[kk].shape[2:], mode='bilinear', align_corners=False)
                # in2 = F.interpolate(in2, size=diffs[kk].shape[2:], mode='nearest', align_corners=False)
            # 增加脸部区域的权重
            diffs[kk] = diffs[kk] * (1 + alpha * in2 * step_ratio)
