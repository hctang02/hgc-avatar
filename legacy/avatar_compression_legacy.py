...
class AvatarNet(nn.Module):
    ...
    def render(self, items, bg_color = (0., 0., 0.), use_pca = False, use_vae = False):
        ...
        folder_output = "output-compress-test2"
        ...
        # plan 2 : 保存为img 形式
        # os.makedirs(f"{folder_output}/pose_map_img", exist_ok=True)
        # save_pose_map1 = pose_map
        ...
        # cv.imwrite(f"{folder_output}/pose_map_img/pose_map_{items['item_idx']}.jpg", save_pose_map1)
        # plan 3 : 保存为txt 格式
        # import pandas as pd
        # pose_map_npy = pose_map.cpu().detach().numpy()
        ...
        pose_map_img = cv.imread(f"{folder_output}/pose_map_decode/pose_map_{timestamp}.jpg", cv.IMREAD_COLOR)
        pose_map_tensor = torch.tensor(pose_map_img, dtype=torch.float32) / 255.0
        os.remove(f"{folder_output}/pose_map_decode/pose_map_{timestamp}.jpg")
        # os.remove(f"output2/pose_map/pose_map_{timestamp}.jpg")
        # 确保恢复为 (3, 512, 512)
        ...
        if timestamp % 1000 == 200:
            # Image.fromarray(color_map_img).save(f"output/color_map/color_map_{timestamp}.png")  # 保存为 PNG 格式
            cv.imwrite(f"{folder_output}/color_map/color_map_{timestamp}.png", save_color_map.cpu().numpy())
