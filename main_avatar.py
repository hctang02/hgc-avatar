...
class AvatarTrainer:
    def __init__(self, opt):
        ...
        # self.calculate_mac()  # 新增这一行
        # self.avatar_net.compute_flops(input_shape=(3, 512, 512))  # 或根据你实际输入尺寸修改
        # flops = FlopCountAnalysis(self.avatar_net, torch.randn(1, 3, 512, 512).to(config.device))
        # print(f"FLOPs: {flops.total() / 1e9:.2f} GMACs")
    ...
    def calculate_mac(self):
        ...
        print(f"AvatarNet Parameters: {params / 1e6:.2f} M")

    # def forward(self, x):
    # # 为了支持FLOPs计算，x 是一个 (B, 3, H, W) 的 pose_map_tensor
    # # 此处假设只调用一个分支，比如 self.color_net
    #     style = self.color_style
    #     return self.color_net([style], x, randomize_noise=False)[0]
    ...
    def compute_lpips_mask_loss(self, image, gt_image, face_mask, item_idx):
        assert image.shape[1] == image.shape[2] and gt_image.shape[1] == gt_image.shape[2]
        lpips_loss = self.lpips.forward_face(
            image[None, [2, 1, 0]],
            gt_image[None, [2, 1, 0]],
            face_mask[None, None], 
            item_idx,
    ...
    def forward_one_pass(self, items):
        ...
        if self.loss_weight['lpips'] > 0.:
            ...
            lpips_loss = self.compute_lpips_mask_loss(image, gt_image, face_mask, items['item_idx'])
    ...
    def train(self):
        ...
        for epoch_idx in range(start_epoch, 9999999):
            ...
            for batch_idx, items in enumerate(dataloader):
                ...
                one_step_end.record()
                torch.cuda.synchronize()  # 确保事件被正确记录
                # torch.cuda.synchronize()
                print('One step costs %f secs' % (one_step_start.elapsed_time(one_step_end) / 1000.))
    ...
    def mini_test(self, pretraining = False, eval_cano_pts = False):
        ...
        gt_image, _ ,_= self.dataset.load_color_mask_images(pose_idx, view_idx)
