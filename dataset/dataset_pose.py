...
class PoseDataset(Dataset):
    ...
    def __init__(
        ...
        if ext == '.pkl':
            smpl_data = joblib.load(open(data_path, 'rb'))
            smpl_data = dict(smpl_data)
            self.body_poses = torch.from_numpy(smpl_data['pose']).to(torch.float32)
            self.transl = torch.from_numpy(smpl_data['pred_cam']).to(torch.float32) * 1e-3
            ...
            potential_datasets = ['thuman4', 'actorshq', 'avatarrex', 'AMASS', 'customsmpl']
            ...
            if self.dataset_name == 'thuman4' or self.dataset_name == 'actorshq' or self.dataset_name == 'avatarrex':
                ...
                smpl_data = dict(smpl_data)
            elif self.dataset_name == 'AMASS':  # AMASS dataset
                ...
                smpl_data = {
                    ...
                    'left_hand_pose': pose_file['poses'][:, 25 * 3: 40 * 3],
                    'right_hand_pose': pose_file['poses'][:, 40 * 3: 55 * 3]
                    # 'left_hand_pose': pose_file['poses'][:, 22 * 3: 37 * 3], #原始的
                    # 'right_hand_pose': pose_file['poses'][:, 37 * 3:] #原始的
                }
                ...
                smpl_data = {
                    'betas': np.zeros((1, 10), np.float32),
                    'global_orient': pose_file['pose'][:, :3],
                    'transl': pose_file['orig_cam_t'],
                    'body_pose': pose_file['pose'][:, 3: 66],
                }
                ...
                smpl_data['body_pose'][:, 14 * 3] = 0.
                # # 只固定 X 轴旋转角度我自己加的
                # smpl_data['body_pose'][:, 16 * 3] = 0.  # 左肘 X 轴
                # smpl_data['body_pose'][:, 17 * 3] = 0.  # 右肘 X 轴
