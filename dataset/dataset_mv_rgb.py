...
class MvRgbDatasetBase(Dataset):
    ...
    def getitem(self, index, training = True, **kwargs):
        ...
        if training:
            ...
            color_img, mask_img ,face_mask = self.load_color_mask_images(pose_idx, view_idx)
...
class MvRgbDatasetTHuman4(MvRgbDatasetBase):
    ...
    def load_color_mask_images(self, pose_idx, view_idx):
        ...
        face_mask_img = cv.imread(self.data_dir + '/images/cam%02d_mask/%08d.jpg' % (view_idx, pose_idx), cv.IMREAD_UNCHANGED)
        return color_img, mask_img, face_mask_img
...
class MvRgbDatasetAvatarReX(MvRgbDatasetBase):
    ...
    def load_color_mask_images(self, pose_idx, view_idx):
        ...
        face_mask_img = cv.imread(self.data_dir + '/%s_mask/%08d.jpg' % (cam_name, pose_idx), cv.IMREAD_UNCHANGED)
        return color_img, mask_img, face_mask_img
        # return color_img, mask_img
...
class MvRgbDatasetActorsHQ(MvRgbDatasetBase):
    ...
    def load_color_mask_images(self, pose_idx, view_idx):
        ...
        face_mask_img = cv.imread(self.data_dir + '/4x/rgbs/%s_mask/%s_rgb%06d.jpg' % (cam_name, cam_name, pose_idx), cv.IMREAD_UNCHANGED)
        return color_img, mask_img, face_mask_img
        # return color_img, mask_img
